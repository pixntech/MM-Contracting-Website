    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(
        (certificate) =>
          ...
          certificate.organization.toLowerCase().includes(query) ||
    ...
    <div className="pt-28 pb-24">
      ...
      >
        ...
        <div className="content-container relative z-10 text-center">
          ...
          >
            {t('certificates.pageDescription')}
          </motion.p>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-10">
          <div className="flex flex-wrap gap-2">
            {certificateCategories.map((category) => (
              ...
              >
                {t(catLabelKeys[category.id] || category.label)}
              </button>
            ))}
          </div>
          ...
          >
            {filtered.map((certificate, index) => (
              <ScrollReveal key={certificate.id} delay={index * 0.05}>
                <div onClick={() => setSelected(certificate)} className="cursor-pointer">
