...
Write-Host ""
Write-Host "MM Real Projects Timeline Patch" -ForegroundColor Cyan
...
foreach ($rel in $deleteFiles) {
    ...
    if (Test-Path $target) {
        ...
        Write-Host "[REMOVED DUMMY] $rel" -ForegroundColor Yellow
    }
}
...
Write-Host ""
Write-Host "Real projects timeline applied successfully." -ForegroundColor Cyan
Write-Host "Dummy company-history milestones were removed." -ForegroundColor Cyan
